    ...
    for m=1:12
        ...
        for d=1:31
            if d<=eomday(years(y),m)
                if ver=='2.1'
				    url=['https://data.cci.ceda.ac.uk/thredds/dodsC/esacci/lakes/data/lake_products/L3S/v' ver '/merged_product/'...
                    num2str(years(y)) '/' num2str(m,'%02u') '/ESACCI-LAKES-L3S-LK_PRODUCTS-MERGED-' ...
                    num2str(years(y)) num2str(m,'%02u') num2str(d,'%02u') '-fv' ver '.0.nc?'...
